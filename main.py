...
symptoms_dict = {
    ...
    'belly_pain': 100, 'abnormal_menstruation': 101, 'dischromic _patches': 102,
...
diseases_list = {
    ...
    5: 'Arthritis', 0: '(vertigo) Paroymsal  Positional Vertigo',
    2: 'Acne', 38: 'Urinary tract infection', 35: 'Psoriasis', 27: 'Impetigo'
}

# =========================
# Load model BEFORE routes run
# =========================
...
def helper(dis):
    ...
    die = [d for d in die_series.values]

    # workout
    wrkout_series = workout[workout['disease'] == dis]['workout']
    wrkout = [w for w in wrkout_series.values]
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        if item in symptoms_dict:  # avoid KeyError on wrong symptom
...
def home():
    if request.method == 'POST':
        ...
        dis_des, pre, medications_list, rec_diet, workout_list = helper(predicted_disease)

        my_precautions = []
        if pre:  # pre is list of rows; take first row
            for i in pre[0]:
