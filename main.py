...
# ============================================================
# LOAD DATASETS
# Make sure these CSV files are in the same folder as main.py
# ============================================================
try:
    ...
    # You might want to handle this gracefully, but for now print error
...
def helper(dis):
    ...
    # Precautions - FIXED: converting to list and removing NaNs
    pre = precautions[precautions['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    pre = pre.values.flatten()
    pre = [p for p in pre if pd.notna(p)]  # Filter out NaN values

    # Medications
    med = medications[medications['Disease'] == dis]['Medication']
    med = med.values.flatten().tolist() if not med.empty else []

    # Diets
    die = diets[diets['Disease'] == dis]['Diet']
    die = die.values.flatten().tolist() if not die.empty else []

    # Workout
    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = wrkout.values.flatten().tolist() if not wrkout.empty else []
...
def get_predicted_value(patient_symptoms):
    ...
    # FIXED: Added safety check to prevent crashing if a symptom isn't in the dict
...
def home():
    if request.method == 'POST':
        ...
        # Split user input by comma and clean spaces
